    ...
    try {
      const result = await pool.query(`
        ...
        WHERE dr.reminder_date = CURRENT_DATE
          ...
          AND dr.popup_created = false
      `);

      for (const reminder of result.rows) {
        const emailSubject = `Project Deadline Reminder (${reminder.reminder_days_before} Days Left)`;

        const emailMessage =
          `Reminder for project document:\n\n` +
          `Document: ${reminder.file_name}\n` +
          `Deadline: ${new Date(reminder.deadline_date).toDateString()}\n\n` +
        ...
        if (emailSent) {
          ...
          console.log(`Reminder sent for document ${reminder.file_name}`);
        }
      }
    } catch (error) {
